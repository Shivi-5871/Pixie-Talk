...
def main():
    ...
    if step == "generate-story":
        if len(args) < 1:
            print("Missing prompt for story generation.")
            return
        ...
        if len(args) < 3:
            print("Missing parameters for image generation.")
            return
        ...
        print(f" Generating {images} images for {scenes} scenes with style {style}")
        ...
        if len(args) < 3:
            print(" Missing parameters for merging.")
            return
        ...
        print(f" Merging {audio} and {video} into {output_file}")
        final_output = merge_videos_and_audio([video], audio, output_file)

        if final_output:
            print(f"Merged video saved to {final_output}")
        else:
            print("Failed to merge videos and audio.")
